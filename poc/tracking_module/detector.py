...
from kafka_util import consumers
import os
import json
import cv2
import numpy as np
...
class Detector:
    ...
    def detect_for_tracking(self, image, debug=False):
        """DeepSort 호환 형식으로 검출 결과 반환"""
        boxes, scores, class_ids, timing_info = self.infer(image, debug)
        ...
        for box, score, class_id in zip(boxes, scores, class_ids):
            ...
            detections.append(([x1, y1, w, h], float(score), class_name))
    ...
    def run(self):
        for msg in self.frame_consumer:
            try:
                # 4-1) camera_id 추출 (key 디코딩)
                camera_id = msg.key.decode('utf-8')

                # 4-2) JPEG 바이트 → OpenCV 이미지로 디코딩
                ...
                # 4-3) Detector 실행 (YOLO inference)
                #    bbs: [([x, y, w, h], confidence, class_id), ...]
                bbs = self.detect(frame)

                # 4-4) JSON 직렬화 가능한 형태로 변환
                detections = []
                for (x, y, w, h), conf, cls in bbs:
                    detections.append({
                ...
                # 4-5) Kafka 전송 (key = camera_id → 파티션 라우팅)
